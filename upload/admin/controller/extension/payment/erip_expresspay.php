...
class ControllerExtensionPaymentEripExpresspay extends Controller
{
  ...
  public function index()
  {
    ...
    $data = $this->model_extension_payment_erip_expresspay->setParametersFromConfig($this->config, $this->request->post, $data);
    ...
    $this->response->setOutput($this->load->view('extension/payment/erip_expresspay', $data));
  }
  ...
  {
    $data['action'] = $this->url->link('extension/payment/erip_expresspay', 'user_token=' . $this->session->data['user_token'], true);
  ...
  private function setBreadcrumbs($data)
  {
    ...
    $data['breadcrumbs'][] = array(
      ...
      'href'      => $this->url->link('extension/payment/erip_expresspay', 'user_token=' . $this->session->data['user_token'], true),
      'separator' => ' :: '
    );
