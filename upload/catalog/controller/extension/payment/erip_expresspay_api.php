...
class ControllerExtensionPaymentEripExpressPayApi extends Controller{
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($useSignatureForNotification) {
                ...
                if ($valid_signature == $signature) {
                    ...
                    $this->model_extension_payment_erip_expresspay_log->log_error("notify", "Access is denied");
                    return;
                }
            }
            else{
    ...
    private function notify_success($dataJSON)
    {
        ...
        try {
            ...
            $this->model_extension_payment_erip_expresspay_log->log_error("notify_success", "Failed to decode data");
            return;
        }
        ...
        if(isset($accountNo)){
            ...
            switch ($cmdtype) {
                case 1:
                    ...
                    $this->model_extension_payment_erip_expresspay_log->log_info("notify_success", "the notice is processed");
                    return;
                case 2:
                    ...
                    if(isset($status)){
                        switch($status){
                            ...
                            default:
                                ...
                                $this->model_extension_payment_erip_expresspay_log->log_error("notify_success", "Invalid status; Status - ".$status);
                                return;
                        }
                        header("HTTP/1.1 200 OK");
                        echo 'OK | the notice is processed';
                ...
                default:
                    ...
                    $this->model_extension_payment_erip_expresspay_log->log_error("notify_success", "Invalid cmdtype; CmdType - ".$cmdtype);
                    return;
                }
        }
        header('HTTP/1.1 400 Bad Request');
        echo 'FAILED | The notice is not processed';
        $this->model_extension_payment_erip_expresspay_log->log_error("notify_success", "The notice is not processed");
    }
