...
class ControllerExtensionPaymentEripExpressPay extends Controller{
    ...
    public function index()
    {
        ...
        $this->load->model('extension/payment/erip_expresspay_log');
        ...
        $this->model_extension_payment_erip_expresspay_log->log_info("index", "DATA: " . json_encode($data));
    ...
    public function success()
    {
        ...
        $this->model_extension_payment_erip_expresspay_log->log_info("successStart", "Order Id: " . $this->session->data['order_id']);
        ...
        if ($this->config->get(self::IS_SHOW_QR_CODE_PARAM_NAME)  == 'on' && isset($this->request->get['ExpressPayInvoiceNo'])) {
            ...
            try {
                $qrbase64 = $this->model_extension_payment_erip_expresspay->getQrbase64($invoiceNo, $this->config);
            } catch (Exception $e) {
                $this->model_extension_payment_erip_expresspay_log->log_error_exception('success', 'Get response; INVOICE ID - ' . $this->session->data['order_id'] . '; RESPONSE - ' . $qrbase64, $e);
                return;
            }
            try {
                $qrbase64 = json_decode($qrbase64);
            } catch (Exception $e) {
                $this->model_extension_payment_erip_expresspay_log->log_error_exception('success', 'Get response; ORDER ID - ' . $this->session->data['order_id'] . '; RESPONSE - ' . $qrbase64, $e);
            }
        ...
        $this->model_extension_payment_erip_expresspay_log->log_info("successFinish", "DATA: " . json_encode($data));
    ...
    public function fail()
    {
        $this->load->model('extension/payment/erip_expresspay_log');
        $this->load->language('extension/payment/erip_expresspay');
        $this->model_extension_payment_erip_expresspay_log->log_info("failStart", "Order Id: " . $this->session->data['order_id']);
        ...
        $this->model_extension_payment_erip_expresspay_log->log_info("failFinish", "DATA: " . json_encode($data));
